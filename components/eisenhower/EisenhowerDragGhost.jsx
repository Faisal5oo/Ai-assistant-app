 ...
 */
export function EisenhowerDragGhost({
  title,
  initialX,
  initialY,
  registerMover,
  ...
  return (
    <div
      ref={rootRef}
      className="pointer-events-none fixed z-[200] w-60 will-change-transform"
      style={{ left: initialX, top: initialY }}
    >
      <motion.div
        ref={cardRef}
        className="flex select-none touch-none items-center gap-2 rounded-2xl border border-white/80 bg-white/95 px-4 py-3 shadow-soft backdrop-blur-md will-change-transform"
        style={{
          transform: "translate(-50%, -50%) rotate(0deg) scale(1.04)",
        }}
      >
        <GripVertical size={14} className="shrink-0 text-gold/70" />
        <span className="line-clamp-1 text-sm font-semibold tracking-tight text-charcoal">
          {title}
        </span>
      </motion.div>
    </div>
