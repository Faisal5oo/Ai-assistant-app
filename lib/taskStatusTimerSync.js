...
import { purgeFocusSession } from "@/lib/focusSessionSync";
...
/** Hard-reset timer without recording session time (e.g. deleted task). */
...
export function finalizeActiveTimerSession(recordToDb = true) {
  ...
  if (recordToDb && sessionMs > 0) {
    recordTaskTimeImperative(taskId, sessionMs, todayKey(), {
      startedAt: new Date(Date.now() - sessionMs).toISOString(),
