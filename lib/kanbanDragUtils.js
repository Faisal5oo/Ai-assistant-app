...
/**
 * Apply a column reorder (including cross-column moves) without duplicating tasks.
 * @param {import('@/types/interfaces').Task[]} tasks
 * @param {import('@/types/interfaces').TaskStatus} columnId
 * @param {string[]} taskIds
 */
export function applyColumnReorder(tasks, columnId, taskIds) {
  const idSet = new Set(taskIds);
  const remaining = tasks.filter((t) => !idSet.has(t.id));
  const nextColumn = taskIds
    .map((tid) => {
      const task = tasks.find((t) => t.id === tid);
      return task ? { ...task, status: columnId } : null;
    })
    .filter(Boolean);

  return STATUS_ORDER.flatMap((st) =>
    st === columnId ? nextColumn : remaining.filter((t) => t.status === st)
  );
}
...
export function resolveDropIndex(clientX, clientY, zoneStatus, draggingTaskId) {
  ...
  const dropTail = list.querySelector("[data-kanban-drop-tail]");
  ...
  if (dropTail) {
    const tailRect = dropTail.getBoundingClientRect();
    if (clientY >= tailRect.top) {
      return items.length;
    }
  }

  if (clientY <= listRect.bottom || clientY <= zoneRect.bottom) {
    return items.length;
  }
