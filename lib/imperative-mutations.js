...
import { syncCompletionTimeCaches } from "@/lib/completion-cache-sync";
import {
...
export async function updateTaskImperative(id, updates, options = {}) {
  ...
  try {
    const { task, dailyLogs } = await tasksApi.update(id, toApiTaskUpdates(updates));
    setTasksInCache((tasks) =>
      tasks.map((t) => (t.id === task.id ? task : t))
    );
    syncCompletionTimeCaches(prevTask, task, dailyLogs);
    return task;
  } catch (error) {
...
export async function completeTaskImperative(taskId) {
  const previous = getTasksFromCache();
  const prevTask = previous.find((t) => t.id === taskId);
  ...
  try {
    const { task, dailyLogs } = await productivityApi.completeTask(taskId);
    setTasksInCache((tasks) =>
      tasks.map((t) => (t.id === task.id ? task : t))
    );
