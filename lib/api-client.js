 ...
 * @returns {string}
 */
function buildTasksQuery(params = {}) {
  const search = new URLSearchParams();
  if (params.scope) search.set("scope", String(params.scope));
  if (params.localDate) search.set("date", String(params.localDate));
  if (params.tzOffset != null) search.set("tzOffset", String(params.tzOffset));
...
export const tasksApi = {
  ...
  async list(options = {}) {
    const res = await fetch(
      `/api/tasks${buildTasksQuery(options)}`,
      defaultFetchOptions
    );
  ...
  async update(id, updates) {
    ...
    const data = await parseJson(res);
    if (!res.ok || !data.success) apiError(res, data);
    return { task: data.task, dailyLogs: data.dailyLogs ?? null };
  },
  ...
  async reorder(payload) {
    ...
    const data = await parseJson(res);
    if (!res.ok || !data.success) apiError(res, data);
    return { success: true, dailyLogs: data.dailyLogs ?? null };
  },
